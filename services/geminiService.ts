...
export const testApiKeyConnection = async (): Promise<{ success: boolean; message: string; details?: string }> => {
  ...
  try {
    // We use a simple text-only model for the fastest and cheapest connectivity test
    ...
    if (response.text) {
      return { success: true, message: "연결 성공: API 키가 활성화되어 있으며 정상 작동 중입니다." };
    }
    return { success: false, message: "응답 수신 실패: 알 수 없는 오류가 발생했습니다." };
  } catch (error: any) {
    let msg = error.message || JSON.stringify(error);
    let details = "";
    
    if (msg.includes('403') || msg.includes('permission')) {
      details = "결제 계정이 연결된 유료 프로젝트의 API 키가 아니거나 권한이 없습니다. ai.google.dev에서 결제 설정을 확인하세요.";
    } else if (msg.includes('429')) {
      details = "할당량 초과: 단시간에 너무 많은 요청을 보냈거나 할당량이 부족합니다.";
    } else if (msg.includes('401') || msg.includes('API_KEY_INVALID')) {
      details = "유효하지 않은 API 키입니다. 키를 다시 선택해 주세요.";
    } else {
      details = "네트워크 오류 또는 일시적인 서버 문제입니다.";
    }
